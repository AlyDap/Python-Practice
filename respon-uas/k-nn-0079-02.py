...
# Minta pengguna untuk memasukkan nilai n_neighbors
valid_values = [1, 3, 5, 7]
while True:
    try:
        n_neighbors_input = int(input("Masukkan nilai n_neighbors (1, 3, 5, 7): "))
        if n_neighbors_input in valid_values:
            break
        else:
            print("Masukkan nilai yang benar (1, 3, 5, atau 7).")
    except ValueError:
        print("Masukkan nilai yang benar (angka positif).")

# Panggil fungsi k_nn_prediction dengan nilai n_neighbors yang dimasukkan
k_nn_prediction(X_train, y_train, X_test, y_test, n_neighbors_input)